...
const ThemeSelector: React.FC<ThemeSelectorProps> = ({ currentTheme, onThemeChange, language = 'en' }) => {
    ...
    <div className="flex items-center gap-3 p-1.5">
      <div className="flex gap-2 pr-2 overflow-x-auto no-scrollbar">
        {THEMES.map((theme) => {
          ...
          const label = themeText.name || theme.label;
          return (
          <button
            key={theme.id}
            ...
            className={`w-9 h-9 rounded-full border-2 transition-all flex-shrink-0 flex items-center justify-center relative group ${
              currentTheme === theme.id ? 'border-[var(--primary)] scale-110 shadow-lg' : 'border-transparent hover:scale-105'
            }`}
          >
            <div className={`w-full h-full rounded-full overflow-hidden flex transform rotate-45`}>
                <div className={`w-1/2 h-full ${theme.colors[0]}`} />
                <div className={`w-1/2 h-full ${theme.colors[1]}`} />
            </div>
            {currentTheme === theme.id && (
              <div className="absolute inset-0 flex items-center justify-center">
                <Check className={`w-3 h-3 drop-shadow-md ${theme.id.startsWith('dark') ? 'text-white' : 'text-slate-800'}`} />
              </div>
            )}
