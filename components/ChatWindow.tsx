...
try {
  ...
  if (
    ...
    safeMessage = "The AI is busy right now. Please retry in 30–60 seconds.";
  }
  ...
  const personas: { id: ResponseStyle; label: string; icon: any; color: string }[] = [
    { id: 'compassionate', label: 'Warm', icon: Heart, color: 'text-pink-500' },
    { id: 'direct', label: 'Direct', icon: Target, color: 'text-orange-500' },
    { id: 'scientific', label: 'Clinical', icon: Microscope, color: 'text-blue-500' },
    { id: 'reflective', label: 'Reflective', icon: HelpCircle, color: 'text-purple-500' },
  ];
    ...
    <div className="flex flex-col h-[650px] theme-card rounded-[48px] shadow-2xl border border-[var(--border)] overflow-hidden transition-theme">
          ...
          <div>
            ...
            <div className="flex flex-wrap items-center gap-2 mt-1.5">
               <span className={`flex items-center gap-1.5 text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded-lg border transition-colors ${isBackendOnline ? 'bg-emerald-500/10 text-emerald-600 border-emerald-500/20' : 'bg-slate-500/10 text-slate-400 border-slate-500/20'}`}>
                  <Database className="w-2.5 h-2.5" /> {isBackendOnline ? 'Local Knowledge' : 'Cloud Only'}
               </span>
        ...
        {messages.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center text-center space-y-6 max-w-sm mx-auto">
            ...
            <div className="space-y-3">
               <h4 className="text-2xl font-black theme-text-main">Intelligence Center Ready</h4>
               <p className="theme-text-muted text-sm leading-relaxed font-medium">
                  {isBackendOnline 
                    ? "Connected to Local Knowledge Base. I'm ready to provide context-aware support." 
                    : "Direct Cloud AI active. Backend not detected on localhost:8000."}
               </p>
            </div>
          </div>
        )}
        ...
        {isLoading && messages[messages.length-1]?.role !== 'model' && (
          <div className="flex justify-start">
            <div className="flex gap-4 items-center theme-text-muted">
              ...
              <span className="text-xs font-black uppercase tracking-widest">Generating {responseStyle} response...</span>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="p-8 border-t border-[var(--border)] bg-[var(--card-bg)]/80 backdrop-blur-md">
        ...
        <div className="flex justify-between items-center mt-4 px-4">
           <div className="flex items-center gap-4">
              <p className="text-[8px] theme-text-muted font-black uppercase tracking-[0.2em]">Active Style: {responseStyle}</p>
              <div className="flex items-center gap-1.5">
                ...
                <span className={`text-[8px] font-black uppercase tracking-[0.2em] ${isBackendOnline ? 'text-emerald-500' : 'text-blue-400'}`}>
                  {isBackendOnline ? 'Local Host Linked' : 'Direct Cloud AI'}
                </span>
              </div>
           </div>
           <p className="text-[8px] theme-text-muted font-black uppercase tracking-[0.2em]">Powered by Gemini 3 Pro</p>
        </div>
      </div>
    </div>
  );
